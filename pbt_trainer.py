...
class Trainer(Trainable):
    ...
    def _setup(self, config):
        from pbt_train_bilstm import BiLSTMClassifier

        self.device = torch.device("cuda" if not config["args"]["no_cuda"] and torch.cuda.is_available() else "cpu")
        set_seed(config["args"]["seed"])

        self.train_dataset, self.val_dataset, text_field = load_data(config["args"]["data_dir"], spacy_tokenizer, augmented=config["args"]["augmented"], use_teacher=config["args"]["use_teacher"], vector_cache=config["vector_cache"])
        vocab = text_field.vocab

        self.model = BiLSTMClassifier(2, len(vocab.itos), vocab.vectors.shape[-1],
            lstm_hidden_size=300, classif_hidden_size=400, dropout_rate=0.15).to(self.device)
        # Initialize word embeddings to fasttext
        self.model.init_embedding(vocab.vectors.to(self.device))


        # Storing
        self.loss_option = config["args"]["loss"]
        self.temperature = config["args"]["temperature"]
        self.max_grad_norm = config["args"]["max_grad_norm"]
        self.batch_size = config["args"]["batch_size"]
        self.gradient_accumulation_steps = config["args"]["gradient_accumulation_steps"]
        self.weight_decay = config["args"]["weight_decay"]
        self.lr = config["lr"]
    ...
    def _train(self):
        for batch in self.train_it:
            self.train_step(batch)

        return {"accuracy": self.evaluate()["accuracy"], "lr": self.lr}

    def evaluate(self):
        self.model.eval()
        val_loss = val_accuracy = 0.0
        loss_func = nn.CrossEntropyLoss(reduction="sum")
        for batch in self.val_it:
            with torch.no_grad():
